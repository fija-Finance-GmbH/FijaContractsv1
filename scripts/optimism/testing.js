...
const main = async () => {
  ...
  LiquidityExamples = await ethers.getContractFactory("UniswapV3InteractionOp")
  ...
  Aave = await ethers.getContractFactory("AaveV3InteractionOp")
  aave = await Aave.deploy()
  await aave.deployed()
  console.log(`The Aave address is: ${aave.address}`)

  straCost = "100000000" //100
  strategy = await ethers.getContractFactory("FijaStrategyTestOp")
  ...
  daiWhale = await ethers.getSigner(DAIop_WHALE)
  usdcWhale = await ethers.getSigner(USDCop_WHALE)
  usdtWhale = await ethers.getSigner(USDTop_WHALE)
  dai = await ethers.getContractAt("IERC20", DAIop)
  usdc = await ethers.getContractAt("IERC20", USDCop)
  usdt = await ethers.getContractAt("IERC20", USDTop)
  ...
  const inv = await liquidityExamples
    .connect(accounts[0])
    .mintNewPosition(buyer, uniDAI, DAIop, USDCop)
  await inv.wait()
  await liquidityExamples
    .connect(accounts[0])
    .mintNewPosition(buyer, uniUSDC, USDCop, USDTop)
