...
const main = async () => {
  ...
  LiquidityExamples = await ethers.getContractFactory("UniswapV3Interaction")
  ...
  Aave = await ethers.getContractFactory("AaveV3Interaction")
  aave = await Aave.deploy()
  await aave.deployed()
  console.log(`The Aave address is: ${aave.address}`)

  straCost = "100000000" //100
  strategy = await ethers.getContractFactory("FijaStrategyTest")
  ...
  await liquidityExamples
    .connect(accounts[0])
    .mintNewPosition(buyer, uniUSDC, USDC, USDT)
  ...
  const InvestInAave = await aave.supply()
