...
contract FirstVault is ERC4626, Auth {
    ...
    /// @notice The base unit of the asset token and hence rvToken.
    /// @dev Equal to 10 ** decimals. Used for fixed point arithmetic.
    uint256 internal immutable BASE_UNIT;

    /// @notice Creates a new Vault that accepts a specific asset token.
    ///  _asset The ERC20 compliant token the Vault should accept.

    //STRATEGY????
    //varibales below just added in plane
...
//Not all teh funds that the user invest are directly invested in the strategy. 
//There is the idle amount that are a part of the funds that are sitting on the vault. When the user first withdraws, he gets a part from that idle amount
//if the withdraw it is completly done with this idle portion, there is no fees, but if it is done with a portion of the strategy, then it is a 0.5% fee and 5 % on profits
    constructor(ERC20 _asset, address _controller)
        
        ERC4626(
            // asset token
            _asset,
            // ex: Rari Dai Stablecoin Vault
            string(abi.encodePacked("Fija", _asset.name(), " Vault")),
            // ex: rvDAI
            string(abi.encodePacked("fj", _asset.symbol()))
        )
        Auth(Auth(msg.sender).owner(), Auth(msg.sender).authority())
    {
        ...
        // Prevent minting of fijaTOKENS until
    ...
    function mint(uint256 shares, address to) public override onlyController returns (uint256 amount) {
    ...
    function withdraw(
        ...
        emit Withdraw(from, to, from, amount, shares);
 ...
 /// @notice Self destructs a Vault, enabling it to be redeployed.
    ...
    //TODO: see if we really need it. Why do we want unwrapped ether
